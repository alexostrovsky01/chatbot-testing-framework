...
def rewrite_query(state: RAGState):                                                                               
    ...
    prompt = ChatPromptTemplate.from_template("""Rewrite the user's question into sub-questions for better retrieval:
"{input}". Only write the sub-questions, one per line.""")
...
def handle_request():                                                                                             
    ...
    recorder = LocalJsonRecorder(settings={"filepath": "tests/results/traces_v3.json"})                                 
